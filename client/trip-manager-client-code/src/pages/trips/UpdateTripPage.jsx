...
function parseStops(routeGeoJson) {
  ...
  try {
    ...
    if (Array.isArray(parsed.stops) && parsed.stops.length > 0) return parsed.stops;
  } catch (err) {
...
export default function UpdateTripPage() {
  ...
  const [formData, setFormData] = useState({ title: "", tripDate: "", tripLeaderNationalId: "" });
  ...
  useEffect(() => {
    ...
    async function fetchTrip() {
      try {
        ...
        if (!trip) { navigate("/not-found", { replace: true }); return; }
  ...
  function validate() {
    ...
    if (!String(formData.tripLeaderNationalId || "").trim()) e.tripLeaderNationalId = "אחראי הטיול הוא שדה חובה";
    stops.forEach((stop, i) => {
      if (!stop.name?.trim()) e[`stop_${i}_name`] = `שם עצירה ${i + 1} חסר`;
      if (!stop.type) e[`stop_${i}_type`] = `סוג עצירה ${i + 1} חסר`;
      if (stop.type === "מסלול הליכה" && !stop.trailCondition) e[`stop_${i}_condition`] = `מצב מסלול עצירה ${i + 1} חסר`;
      if (stop.type === "אטרקציה" && !stop.officialApproval?.trim()) e[`stop_${i}_approval`] = `אישור רשמי עצירה ${i + 1} חסר`;
    });
    return e;
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      alert("פרטי הטיול עודכנו בהצלחה! ✓");
      navigate(`/trips/${tripId}`);
    } catch (err) {
      setSubmitError(err.response?.data?.message || err.response?.data || "עדכון הטיול נכשל, נסה שנית");
    } finally {
      setLoading(false);
    }
  }
  ...
  if (fetching) {
    return (
      <>
        <Navbar />
        <main className="page-main"><p style={{ textAlign: "center", padding: "2rem" }}>טוען...</p></main>
      </>
    );
  }
  ...
  return (
    <TripForm
      pageTitle={writeAccess ? `עדכון מסלול — ${formData.title}` : `מסלול הטיול — ${formData.title}`}
      ...
      loading={loading}
      onFieldChange={(e) => setFormData((p) => ({ ...p, [e.target.name]: e.target.value }))}
      onStopChange={(i, s) => setStops((p) => p.map((x, j) => (j === i ? s : x)))}
